...
MNIST::MNIST(string data)
    : file{data}, n{0}, m{0}, magic_number{0}, number_of_items{0},
      mini_batch_size{0}, image_dat_set{nullptr}, label_dat_set{nullptr},
      mini_batch_imag{nullptr}, mini_batch_label{nullptr} {
  ...
  if (image_dat_set != NULL) {
    cout << "\nLoading data set of "
         << ((file == "train") ? "training" : "test") << " images... ";
  ...
  if (label_dat_set != NULL) {
    cout << "Loading data set of " << ((file == "train") ? "training" : "test")
         << " labels... ";
    for (size_t i{0}; i < number_of_items; i++) {
...
MNIST::MNIST(const MNIST &data)
    : file{data.file}, n{data.n}, m{data.m}, magic_number{data.magic_number},
      number_of_items{data.number_of_items}, mini_batch_size{
                                                 data.mini_batch_size} {
  ...
  if (image_dat_set != NULL && label_dat_set != NULL &&
      mini_batch_imag != NULL && mini_batch_label != NULL) {
    ...
    memcpy(mini_batch_imag, data.mini_batch_imag,
           number_of_items * sizeof(Matrix));
    memcpy(mini_batch_label, data.mini_batch_label,
           number_of_items * sizeof(Matrix));
  } else {
    cerr << "Error allocating dynamic memory!\n" << endl;
    exit(1);
...
// Get number of items
int MNIST::get_number_items() { return number_of_items; }

// Get image
Matrix MNIST::getImage(int index) { return image_dat_set[index]; }

// Get label
Matrix MNIST::getLabel(int index) { return label_dat_set[index]; }
  ...
  if (mini_batch_imag != NULL && mini_batch_label != NULL) {
    for (size_t i{0}; i < number_of_items / mini_batch_size; i++) {
      for (int j{index}; j < index + mini_batch_size; j++) {
        mini_batch_imag[i * mini_batch_size + (j % mini_batch_size)] =
            image_dat_set[j];
        mini_batch_label[i * mini_batch_size + (j % mini_batch_size)] =
            label_dat_set[j];
      }
...
int MNIST::get_mini_batch_size() { return mini_batch_size; }
