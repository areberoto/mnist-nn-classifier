...
Network::Network(int *size_layers)
    : num_layers{3}, performance{0}, biases{nullptr}, weights{nullptr},
      nabla_b{nullptr}, nabla_w{nullptr}, delta_nabla_b{nullptr},
      delta_nabla_w{nullptr}, training_data{"train"}, test{"test"} {
...
void Network::SGD(int epochs, int mini_batch_size, float eta) {
  for (size_t i{0}; i < epochs; i++) {
    ...
    for (size_t j{0}; j < training_data.get_number_items() / mini_batch_size;
         j++)
...
void Network::updateMiniBatch(int mini_batch_index, float eta) {
  ...
  weights[0] =
      weights[0] - (nabla_w[0] * (eta / training_data.get_mini_batch_size()));
  biases[0] =
      biases[0] - (nabla_b[0] * (eta / training_data.get_mini_batch_size()));
  weights[1] =
      weights[1] - (nabla_w[1] * (eta / training_data.get_mini_batch_size()));
  biases[1] =
      biases[1] - (nabla_b[1] * (eta / training_data.get_mini_batch_size()));
}
...
void Network::classify() {
  ...
  cout << (correct * 100.0 / 450.0) << "% of digits were correctly classified!"
       << endl;
